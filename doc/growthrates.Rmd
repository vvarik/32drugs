...
[Growthrates](https://cran.r-project.org/web/packages/growthrates/vignettes/Introduction.html), an R package that helps to do all sort of growth rate estimations.
...
Growthrates contains multiple options to estimate growthrates. Broadly speaking, there are two options:
...
datasets, `bactgrowth`. Dataset contains bacterial growth measurements upon
different concentrations of tetracycline antibiotic. We are concerned with the
variables:
...
The `growthrates` function to estimate growth rate from the linear part of log
...
For this particular data, the Baranyi & Roberts model is not the best fit (or if you find better starting parametes & limits, let me know vvarik.mail@gmail.com).
